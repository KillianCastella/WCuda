...
/*--------------------------------------*\
 |*		Public			*|
 \*-------------------------------------*/
Mandelbrot::Mandelbrot(uint w, uint h, int nMin,int nMax, const DomaineMath& domaineMath) :
	Animable_I<uchar4>(w, h, "Mandelbrot_OMP_rgba_uchar4", domaineMath), variateurAnimation(Interval<float>(20, 120),0)
    {
    // Input
    this->n = n;
...
void Mandelbrot::processForAutoOMP(uchar4* ptrTabPixels, uint w, uint h, const DomaineMath& domaineMath)
    {
    Mandelbrot Mandelbrot(n); // ici pour preparer cuda
